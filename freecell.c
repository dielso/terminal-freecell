...
void SetColor(int ForgC) // função para mudar a cor do terminal
 {
     WORD wColor;
...
int moveTempMesa() // temp para mesa
{
    ...
    if(mesa[pos2] != NULL){ // se a mesa não estiver vazia e...
        ...
        if((temp[pos1]->carta[0]+1 != mesa[pos2]->carta[0]) || // se a carta da origem não for a anterior -> fora
...
int moveMesaMesa() // mesa para outra
{
    ...
    if(op == 1){
        ...
        if(mesa[pos2] != NULL){ // se a mesa destino não estiver vazia e...
            if((mesa[pos1]->carta[0]+1 != mesa[pos2]->carta[0]) || // se a carta da origem não for a anterior -> fora
        ...
        for(i=1; i<qtd;i++){
            ...
            else return 0; // senão, entrada inválida
        }



        if(mesa[pos2] != NULL){ // se a mesa destino não estiver vazia e...
            if((maior->carta[0]+1 != mesa[pos2]->carta[0]) || // se a carta da origem não for a anterior -> fora
               !(((maior->carta[1] == paus || maior->carta[1] == espadas)&&
                 (mesa[pos2]->carta[1] == copas || mesa[pos2]->carta[1] == ouros))||
               ((maior->carta[1] == ouros || maior->carta[1] == copas)&&
                 (mesa[pos2]->carta[1] == paus || mesa[pos2]->carta[1] == espadas)))) return 0; // se os naipes forem de cores iguais -> fora
